...
class EngineWrapper:
    ...
    def to_readable_value(self, stat: InfoDictKeys, info: InfoStrDict) -> str:
        """Convert the stats from the engine to a format that is easy to read."""
        value = info.get(stat)
        if value is None:
            return "?"
        ...
        if stat == "Nodes":
            try:
                nodes = float(value)
                return self.readable_number(int(nodes))
            except (ValueError, TypeError):
                return str(value)
        elif stat == "Speed":
            try:
                nps = float(value)
                return f"{self.readable_number(int(nps))}nps"
            except (ValueError, TypeError):
                return f"{value}nps"
        elif stat == "Score" or stat == "Evaluation":
            if "#" in str(value):
                return str(value)
            try:
                pawns = float(value) / 100
                return f"+{pawns:.2f}" if pawns > 0 else str(pawns)
            except (ValueError, TypeError):
                return str(value)
        return str(value)
    ...
    def get_stats(self, for_chat: bool = False) -> list[str]:
        ...
        """
        can_index = len(self.move_commentary) > 0 and self.move_commentary[-1]
        ...
        stats = ["Source", "Evaluation", "Depth", "Nodes", "Speed", "Time", "Hashfull", "Tbhits", "Nps", "Winrate", "Cpuload"]
...
def get_egtb_move(board: chess.Board, game: model.Game, lichess_bot_tbs: Configuration,
    ...
    if best_move:
        ...
        wdl_to_score = {2: 9900, 1: 500, 0: 0, -1: -500, -2: -9900}
        comment: chess.engine.InfoDict = {
            "score": chess.engine.PovScore(chess.engine.Cp(wdl_to_score[wdl]), board.turn),
            "string": source
        }
