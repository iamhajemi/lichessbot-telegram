...
class Game:
    ...
    def should_accept_draw(self, config: Configuration) -> bool:
        """Düşük zaman durumunda beraberlik teklifini kabul edip etmeyeceğimizi kontrol eder."""
        # Beraberlik teklifi var mı kontrol et
        opponent_offered_draw = bool(self.state.get(f"{self.opponent_color[0]}draw"))
        if not opponent_offered_draw:
            return False

        # Düşük zaman ayarı aktif mi kontrol et
        if not config.accept_draw_on_low_time:
            return False
            
        # Kalan zamanı kontrol et
        remaining_time = self.state.get(self.my_color[0] + "time", 0)
        return remaining_time <= config.low_time_threshold
